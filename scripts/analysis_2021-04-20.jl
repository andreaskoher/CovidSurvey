...
# ============================================================================
fdir = "/home/and/.julia/dev/CovidSurvey/out/"
fname = "chains_cases=2020-06-01_chains=1_hospit=true_model=v1_observ=2021-03-25_seed=10_sero=true_steps=800_warmup=300.jls"

# ps = CovidSurvey.parseparams(fdir*fname)
# data = National.load_data(ps.observ, nothing, ps.cases, ps.hospit, ps.sero)
# turing_data = data.turing_data;
# model = name2model[ps.model]
chain = chainscat(
    read(fdir*fname, Chains),
    read("/home/and/.julia/dev/CovidSurvey/out/chains_cases=2020-06-01_chains=1_hospit=true_model=v1_observ=2021-03-25_seed=11_sero=true_steps=800_warmup=300.jls", Chains),
    read("/home/and/.julia/dev/CovidSurvey/out/chains_cases=2020-06-01_chains=1_hospit=true_model=v1_observ=2021-03-25_seed=12_sero=true_steps=800_warmup=300.jls", Chains),
)
chain = chain[201:end,:,:]
#------------------------------------------------------------------------------
...
array_priors = (
    :logRt => Normal{Float64},
    :seroprev_σ => Normal{Float64},
    :logit_iar => Normal{Float64},
);
...
function fitsamples(chain)
    ...
    dfits[:logRt] = [Normal{Float64}(mean(d), std(d)*2) for d in dfits[:logRt]]
...
# histogram(chain["logRt[25]"]|>vec, normalize=true)
# plot!(priors[:logRt][1])
...
Random.seed!(ps.seed);
data = National.load_data(ps.observ, ps.preds |> parse_predictors, ps.cases, ps.hospit, ps.sero; update=false)
turing_data = data.turing_data;
#----------------------------------------------------------------------------
# sample model
m = National.model_contacts_fitted(turing_data..., priors, false)
