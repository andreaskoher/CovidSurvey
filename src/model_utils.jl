...
struct WeekdayHolidayObsModel2{T,M,S,A,P,E,W,H,V} <: CovidSurvey.ObservationsModel
    ps::T
    μ::M
	σ::S
    α::A
...
function infection2observation(obsmodel::Union{WeekdayHolidayObsModel, SimpleObsModel})
...
function infection2observation(obsmodel::Union{WeekdayHolidayObsModel2, SimpleObsModel2})
...
function expected!(obsmodel::WeekdayHolidayObsModel2, newly_infecteds)
	@unpack ps, α, expecteds, holidayeffect, weekdayeffect = obsmodel
	@unpack num_regions, delay_length, holidays, weekdays = ps
	...
	for m in 1:num_regions
		...
		expected[1] = 1e-15*newly_infected[1]
		for t = 2:num_time_step
			weekday_holiday_effect = (1 - he * holiday[t]) * we[weekday[t]]
			expected[t] = α * weekday_holiday_effect * sum(newly_infected[τ] * i2o[t - τ] for τ = (t - 1):-1:max(t-delay_length,1))
		end
	end
	return nothing
...
function Distributions.logpdf(obsmodel::CovidSurvey.ObservationsModel, observed)
	...
	for m in 1:num_regions
		...
		_out_of_bounds(μs) && ( return T(-Inf))
		ℓ += _logpdf(ys, μs, ϕ)
	end
	return ℓ
end

_out_of_bounds(x) = !all(isfinite.(x))

# ============================================================================
# pointwise_loglikelihoods
function pointwise_loglikelihoods(obsmodel::CovidSurvey.ObservationsModel, observed)
	@unpack ps, ϕ, expecteds, vectorize = obsmodel
	@unpack populations, starts, stops, num_regions = ps
...
function weekdayeffect!(weekdayeffect, weekdayeffect_simplex::AbstractVector)
	num_days = 7
...
function weekdayeffect!(weekdayeffect, weekdayeffect_simplex::AbstractArray)
	for i in 1:size(weekdayeffect_simplex, 2)
		@inbounds weekdayeffect!(weekdayeffect[i], weekdayeffect_simplex[:,i])
	end
	return nothing
end
# ============================================================================
...
function Distributions.logpdf(obsmodel::SimpleSeroObsModel, observeds)
