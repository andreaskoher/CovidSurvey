@model function model_v2(
	...
	for t = 2:num_time_steps
		expected_daily_cases[t]  = iar[t] * sum(newly_infected[τ] * i2c[t - τ] for τ = (t - 1):-1:max(t-num_i2c,1))
	...
	seroprev_mean[3:4] ~ MvNormal(seroprev_μ, seroprev_σ) # select only early observations
