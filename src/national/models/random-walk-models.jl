...
# 	########### 4.) compare model to observations
# 	## 4.1) observe cases
# 	ϕ_c  ~ truncated(Normal(25, 10), 30, Inf)
...
#
#     ## 4.3) observe deaths
# 	ϕ_d   ~ truncated(Normal(20, 10), 5, Inf)
...
@model function model_cases(
    ...
    cmodel = WeekdayHolidayObsModel(
        casemodel, μ_i2d, iar, ϕ_c, expected_daily_cases, weekdayeffect, holidayeffect
    )
    dmodel = SimpleObsModel(deathmodel, μ_i2d, ifr, ϕ_d, expected_daily_deaths)


	expected_observations!(expected_daily_deaths, dmodel, newly_infected)
	expected_observations!(expected_daily_cases, cmodel, newly_infected)
...
@model function model_deaths(
	...
	deaths!(expected_daily_deaths, θ, μ_i2d, ifr, newly_infected)
	...
	ℓ  = zero(V)
	ℓ += observe_deaths(θ, expected_daily_deaths, ϕ)
