...
@model function model_v2(
    ...
    last_time_step = predict ? num_total_days : num_obs
    ...
    infected              = TV(undef, last_time_step)
    expected_daily_cases  = TV(undef, last_time_step)
    expected_daily_hospit = TV(undef, last_time_step)
	# expected_seropositive = TV(undef, last_time_step)
    cumcases              = TV(undef, last_time_step)
    expected_daily_deaths = TV(undef, last_time_step)
    latent_Rt             = TV(undef, num_Rt_steps)
	Rt                    = TV(undef, last_time_step)
    Rt_adj                = TV(undef, last_time_step)
	...
	iar = logistic.( latent_iar[iar_index[1:last_time_step]] )
	...
	seroprev_mean[1:4] ~ MvNormal(cumcases[seroprev_idx[1:4] .- 14], seroprev_σ[1:4])
...
@model function model_constant_iar_contacts(
    ...
    cumcases              = TV(undef, last_time_step)
    expected_daily_deaths = TV(undef, last_time_step)
    latent_Rt             = TV(undef, last_time_step-lockdown)
	Rt                    = TV(undef, last_time_step)
    Rt_adj                = TV(undef, last_time_step)
	...
	latent_Rt[covariates_start-lockdown:end] .= latent_Rt[covariates_start-lockdown:end] .+ covariates[1:last_time_step-covariates_start+1, :] * α #
	Rt[1:lockdown] .= R0
	Rt[lockdown+1:last_time_step] = link.(latent_Rt)
...
@model function model_cases(
    ...
    # seroprevalence study
	seroprev_mean ~ MvNormal(expected_seropositive[seroprev_idx], seroprev_σ) #DEBUG use only first values
	# seroprev_μ ~ MvNormal(cumcases[seroprev_idx], seroprev_σ)
...
@model function model_gp(
    ...
    cumcases              = TV(undef, last_time_step)
    expected_daily_deaths = TV(undef, last_time_step)
    logRt                 = TV(undef, last_time_step)
    Rt_adj                = TV(undef, last_time_step)
	latent_iar             = TV(undef, last_time_step)
