...
struct SimpleSeroObsModel2{T,E}# <: ObservationsModel
    θ::T
	# σ::S
	expected::E
end
...
# ============================================================================
# semi-parametric model
function mixed!(Rt, θ, predict, latent_Rt, R0, σ_rt, α)
	@unpack lockdown_index, num_observations, link, covariates, covariates_start, num_total_days = θ #rt_step_indices
	predictor = covariates * α

	Rt[1:lockdown_index] .= R0

	for (j,i) in enumerate(lockdown_index+1:covariates_start-1)
		Rt[i] = link(latent_Rt[j])
	end
	n = covariates_start - lockdown_index - 1
	for (j,i) in enumerate(covariates_start:num_observations)
		Rt[i] = link(latent_Rt[j+n] + predictor[j])
	end
	if predict
		Rt[num_observations+1:num_total_days] .= Rt[num_observations]
		# @unpack num_total_days, invlink = θ
...
function Turing.logpdf(obsmodel::SimpleSeroObsModel2, observed)
	@unpack θ, expected = obsmodel
	@unpack index, delay, std, population = θ

	T    = eltype(expected)
	μ    = expected[index .- delay] / population
	dist = MvNormal(μ, std) # select only early observations
	# _unrealistic(μ, observed) && (@error "unrealistic"; return T(Inf))
